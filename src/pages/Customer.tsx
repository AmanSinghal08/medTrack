...
import {
  ...
  Search, X, UserCheck, UserMinus, IndianRupee, Loader2
} from 'lucide-react';
import { CustomerService } from '../api/services/customerService';
import { type Customer, type CustomerPayload } from '../types/app';
...
export default function Customers() {
  ...
  const filteredCustomers = useMemo(() => {
    return customers.filter((c) => {
      const mobile = c.mobile_no || c.mobileNo || c.phone || '';
      const city = c.city || '';
      return (
  ...
  if (loading) {
    return (
      <div className="h-96 flex flex-col items-center justify-center text-slate-400 gap-4">
        ...
        <p className="font-bold tracking-widest uppercase text-xs">Loading Customers...</p>
      </div>
    );
  }
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredCustomers.map((customer) => {
                const mobile = customer.mobile_no || customer.mobileNo || customer.phone || '-';
                const city = customer.city || '-';
                const isActive = customer.is_active ?? customer.isActive ?? true;
                const currentBalance = customer.current_balance ?? customer.currentBalance ?? 0;
                ...
                return (
                  <tr key={customer.id} className="group hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-8 py-5">
                      <div className={`text-sm font-black ${currentBalance > 0 ? 'text-rose-500' : 'text-emerald-600'}`}>
            ...
            <form onSubmit={handleSave} className="p-8 space-y-5">
              <div className="grid grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  <input name="mobileNo" defaultValue={editingCustomer?.mobile_no || editingCustomer?.mobileNo || ''} required className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none transition-all font-medium" />
                </div>
                <div>
              ...
              <div className="flex items-center justify-between bg-emerald-50/50 p-5 rounded-4xl border border-emerald-100/50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <input type="number" name="currentBalance" defaultValue={editingCustomer?.current_balance ?? editingCustomer?.currentBalance ?? 0} className="bg-transparent font-black text-emerald-700 text-lg focus:outline-none w-24" />
                  </div>
                </div>
                <label className="flex items-center gap-3 cursor-pointer">
                  <span className="text-xs font-black text-emerald-700 uppercase tracking-widest">Active</span>
                  <div className="relative">
                    <input type="checkbox" name="isActive" defaultChecked={editingCustomer?.is_active ?? editingCustomer?.isActive ?? true} className="sr-only peer" />
